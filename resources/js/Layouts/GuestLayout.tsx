            ...
            <AnimatePresence>
                {open && (
                    <motion.div
                        ...
                        className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-2xl border border-outline-variant/20 overflow-hidden z-50"
                    >
                        <div className="p-3 border-b border-outline-variant/10">
                            <p className="font-bold text-on-surface text-sm truncate">{auth.user.name}</p>
                            <p className="text-xs text-on-surface-variant truncate">{auth.user.email}</p>
                        </div>
                        <div className="p-1">
                            <Link href={route('dashboard')}
                                onClick={() => setOpen(false)}
                                className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-on-surface hover:bg-surface-container-low transition-colors"
...
export default function GuestLayout({ children, activeNav }: Props) {
    ...
    return (
        <div className="min-h-screen flex flex-col bg-background text-on-background">
            ...
            <nav className="bg-gradient-to-r from-primary/90 to-primary-container/90 backdrop-blur-2xl text-white font-semibold sticky top-0 z-50 shadow-ambient border-b border-white/5">
                <div className="flex justify-between items-center px-4 sm:px-8 h-16 sm:h-20 w-full max-w-7xl mx-auto">
                    ...
                    <div className="flex items-center gap-2">
                        <Link href={route('trips.search')}
                            className="hidden md:flex items-center gap-2 bg-white text-primary px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-surface-container-low hover:shadow-lg transition-all shadow-ambient"
                        >
                            <Bus size={16} /> Acheter un ticket
                        </Link>
                        <button className="relative hover:bg-white/10 rounded-xl p-2.5 transition-colors">
                            <Bell size={18} />
                            <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-status-red-text rounded-full border border-white" />
                        </button>
                        <UserMenu />
                        <button className="md:hidden p-2.5 hover:bg-white/10 rounded-xl transition-colors" onClick={() => setMobileOpen(!mobileOpen)}>
            ...
            <footer className="bg-inverse-surface w-full">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-12 px-4 sm:px-8 py-10 sm:py-16 w-full max-w-7xl mx-auto text-white">
                    <div>
                        ...
                        <p className="text-on-tertiary-container/70 leading-relaxed text-sm">
                    ...
                    <div>
                        ...
                        <ul className="space-y-4 text-on-tertiary-container/70 text-sm">
                            {['À propos', 'Agences', 'Contact', 'FAQ'].map((l) => (
                                <li key={l}>
                                    <a href="#" className="hover:text-primary-fixed-dim transition-colors flex items-center gap-2 group">
                    ...
                    <div>
                        ...
                        <ul className="space-y-5 text-on-tertiary-container/70 text-sm">
                            <li className="flex items-start gap-4">
                                <MapPin size={18} className="text-primary-fixed-dim shrink-0 mt-0.5" />
                                <span>Gare de Ouagadougou, Secteur 10</span>
                            </li>
                            <li className="flex items-center gap-4">
                                <Phone size={18} className="text-primary-fixed-dim shrink-0" />
                                <span className="font-mono">+226 25 00 00 00</span>
                            </li>
                            <li className="flex items-center gap-4">
                                <Mail size={18} className="text-primary-fixed-dim shrink-0" />
                                <span>contact@rahimo.bf</span>
                            </li>
                        </ul>
                        {/* Lien discret back-office */}
                        <Link href={route('login')} className="mt-8 inline-block text-xs text-on-tertiary-container/30 hover:text-on-tertiary-container/60 transition-colors">
                            Accès personnel →
                        </Link>
                    </div>
                </div>
                <div className="border-t border-white/5 text-center py-6 sm:py-8 px-4 text-on-tertiary-container/40 text-xs sm:text-sm font-mono">
