...
import { Armchair, CreditCard, Loader2, Printer, Route, UserPlus } from 'lucide-react';
...
export default function Guichet({ trajets }: Props) {
    ...
    return (
        <div className="h-[calc(100vh-8rem)] flex flex-col">
            <div className="mb-4">
                <h1 className="text-xl font-bold text-on-surface">Vente Guichet — Billetterie</h1>
            ...
            <form onSubmit={submit} className="flex-1 grid grid-cols-12 gap-6 overflow-hidden min-h-0">
                <div className="col-span-12 lg:col-span-8 flex flex-col gap-5 overflow-hidden">
                    <section className="bg-white rounded-xl shadow-ambient border border-surface-container-high p-5 shrink-0">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="font-semibold text-on-surface flex items-center gap-2 text-sm">
                                <Route size={16} className="text-primary" /> Sélection du trajet
                            </h2>
                            <span className="px-3 py-1 bg-primary-fixed text-[11px] font-bold tracking-wider rounded-full text-primary">AUJOURD'HUI</span>
                        </div>
                                ...
                                <button key={t.id} type="button"
                                    ...
                                    className={`p-4 rounded-xl text-left flex flex-col transition-all active:scale-[0.98] ${
                                        selectedTrajet?.id === t.id
                                            ? 'border-2 border-primary bg-primary/5 shadow-ambient'
                                            : 'border border-surface-container-high hover:border-primary/50 bg-white'
                                    }`}
                                >
                                    ...
                                    <span className="font-semibold text-lg mt-1 text-on-surface">
                    ...
                    <section className="flex-1 bg-white rounded-xl shadow-ambient border border-surface-container-high p-5 flex flex-col min-h-0">
                        ...
                        <div className="flex-1 overflow-y-auto bg-surface-container-low/50 rounded-2xl py-6">
                            <div className="max-w-xs mx-auto bg-white rounded-[2.5rem] p-6 shadow-ambient border ">
                                <div className="flex justify-between items-center mb-8 px-4">
                                    <div className="w-10 h-10 bg-surface-container-low rounded-lg flex items-center justify-center text-on-surface-variant text-xs font-bold">🚌</div>
                                    <div className="w-10 h-10 bg-surface-container-low rounded-full flex items-center justify-center text-on-surface-variant text-xs">→</div>
                                </div>
                                        ...
                                        return (
                                            <motion.button key={n} type="button"
                                                ...
                                                className={`h-9 w-9 rounded-lg border flex items-center justify-center text-[11px] font-bold font-mono transition-all ${
                                                    isSelected
                                                        ? 'bg-primary border-primary text-white scale-110 shadow-primary-glow z-10'
                ...
                <div className="col-span-12 lg:col-span-4 flex flex-col gap-5 overflow-hidden">
                    <section className="bg-white rounded-xl shadow-ambient border border-surface-container-high p-6 shrink-0">
                        <h2 className="font-semibold text-on-surface text-sm mb-4 flex items-center gap-2">
                        ...
                        <div className="space-y-3">
                            <div>
                                ...
                                <input type="text" value={data.passenger_name} onChange={(e) => setData('passenger_name', e.target.value)}
                                    ...
                                    className="w-full  rounded-xl px-4 py-3 focus:ring-primary focus:border-primary transition-all text-sm outline-none border"
                                />
                            </div>
                            <div>
                                ...
                                <input type="tel" value={data.passenger_phone} onChange={(e) => setData('passenger_phone', e.target.value)}
                                    ...
                                    className="w-full  rounded-xl px-4 py-3 focus:ring-primary focus:border-primary transition-all text-sm font-mono outline-none border"
                                />
                            </div>
                        </div>
                    </section>

                    <section className="flex-1 bg-admin-bg rounded-xl p-6 flex flex-col shadow-ambient border border-admin-border min-h-0">
                        ...
                        <div className="flex-1 space-y-5 overflow-y-auto">
                                    ...
                                    <button key={`${opt.val}-${i}`} type="button"
                                        ...
                                        className={`flex flex-col items-center justify-center p-3 rounded-xl transition-all active:scale-[0.96] ${
                                            data.payment_method === opt.val
                                                ? 'bg-primary border-2 border-primary shadow-ambient'
                                                : 'bg-admin-card border border-admin-border opacity-60 hover:opacity-100'
                                        }`}
                                    >
                            ...
                            <div className="space-y-3 bg-admin-card/50 p-4 rounded-2xl border border-admin-border/50">
                                <div className="flex justify-between items-center text-on-surface-variant text-sm">
                                    <span>Ticket (Siège {selectedSeat ?? '—'})</span>
                                    <span className="font-bold text-white font-mono">{formatFCFA(price)}</span>
                                </div>
                                <div className="border-t border-admin-border pt-3 flex justify-between items-center">
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            <input type="number" value={received} onChange={(e) => setReceived(parseInt(e.target.value) || 0)}
                                                className="w-full text-2xl font-bold bg-admin-card border-admin-border text-white rounded-xl py-3 pl-4 pr-16 focus:ring-primary focus:border-primary transition-all font-mono outline-none border"
                                            />
                                            <span className="absolute right-4 top-1/2 -translate-y-1/2 font-bold text-on-surface-variant text-sm">CFA</span>
                                        </div>
                                    </div>
                                    <div className={`p-4 rounded-2xl flex justify-between items-center border ${
                                        change >= 0 && received > 0 ? 'bg-primary/10 border-primary/20' : 'bg-red-500/10 border-status-red-ring/20'
                                    }`}>
                                        <div>
                                            <span className="text-[10px] font-extrabold text-primary tracking-widest uppercase block mb-1">MONNAIE À RENDRE</span>
                                            <span className={`text-2xl font-black font-mono ${change >= 0 && received > 0 ? 'text-white' : 'text-status-red-text'}`}>
                        ...
                        <button type="submit" disabled={processing || !selectedSeat || !selectedTrajet}
                            className="mt-5 w-full bg-primary text-white py-4 rounded-2xl font-semibold text-sm shadow-primary-glow hover:bg-primary-container hover:shadow-primary/40 active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-50"
                        >
