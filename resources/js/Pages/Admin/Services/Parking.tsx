...
export default function Parking({ parkings, stats }: Props) {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('admin.services.parking.store'), {
    ...
    const ST = [
        ...
        { label: 'Terminés', val: stats.termine, color: 'text-status-green-text', bg: 'bg-status-green-bg/30' },
        { label: 'Revenu', val: `${stats.revenu.toLocaleString()} FCFA`, color: 'text-status-yellow-text', bg: 'bg-orange-900/30' },
    ];
    ...
    return (
        <div className="w-full max-w-7xl space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-xl font-bold text-white">Parking</h1>
                    <p className="text-admin-muted text-sm mt-0.5">Gestion des stationnements</p>
                </div>
                        ...
                        <tbody>
                            {parkings.map((p) => (
                                <tr key={p.id} className=" hover:bg-white/5 transition-colors">
                                    <td className="p-4 font-semibold text-white">{p.vehicle_registration}</td>
                                    <td className="p-4">
                                        <p className="text-white">{p.driver_name}</p>
                                        <p className="text-admin-muted text-xs">{p.driver_phone}</p>
                                    </td>
                                    <td className="p-4 text-admin-muted">{p.entry_date}</td>
                                    <td className="p-4 text-admin-muted">{p.exit_date ?? '—'}</td>
                                    <td className="p-4 text-right text-white">{p.amount.toLocaleString()} FCFA</td>
                                    ...
                                    <td className="p-4 text-center">
                                        {p.status === 'en_cours' && (
                                            <button onClick={() => router.post(route('admin.services.parking.sortir', p.id))}
                                                className="flex items-center gap-1 mx-auto px-3 py-1.5 bg-status-yellow-text/20 text-status-yellow-text rounded-lg text-xs font-semibold hover:bg-status-yellow-text/30 transition-all"
                            ...
                            {parkings.length === 0 && (
                                <tr><td colSpan={9} className="p-8 text-center text-admin-muted">Aucun stationnement</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                ...
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setShowForm(false)}>
                    <div className="bg-admin-card rounded-2xl border border-white/10 p-6 w-full max-w-lg mx-2 sm:mx-4" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-lg font-bold text-white">Nouveau stationnement</h2>
                        ...
                        <form onSubmit={submit} className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="text-xs text-admin-muted mb-1 block">Entrée *</label>
                                    <input type="datetime-local" value={data.entry_date} onChange={e => setData('entry_date', e.target.value)} required
                                        className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-primary" />
                                </div>
                            </div>
                            <div>
                                <label className="text-xs text-admin-muted mb-1 block">Conducteur *</label>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="text-xs text-admin-muted mb-1 block">Payé (FCFA) *</label>
...
Parking.layout = (page: React.ReactNode) => (
    <BackOfficeLayout title="Parking" breadcrumbs={[{ label: 'Services' }, { label: 'Parking' }]}>
        {page}
    </BackOfficeLayout>
);
