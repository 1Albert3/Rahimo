...
export default function CheckLogs({ logs, stats }: Props) {
    return (
        <div className="w-full max-w-7xl space-y-6">
            <div>
                <h1 className="text-xl font-bold text-white">Journal des Vérifications Police</h1>
                <p className="text-admin-muted text-sm mt-0.5">Historique de toutes les vérifications silencieuses</p>
            </div>
                            ...
                            {logs.data.map(l => {
                                ...
                                return (
                                    <tr key={l.id} className="hover:bg-white/5 transition-colors">
                                        ...
                                        <td className="px-4 py-3 text-admin-muted font-mono">{l.phone ?? '—'}</td>
                                        <td className="px-4 py-3">
                                            <span className={`inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded ${badge.color}`}>
                                                <badge.icon size={10} /> {badge.label}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3 text-admin-muted text-xs">{l.check_type}</td>
                                        <td className="px-4 py-3 text-admin-muted text-xs">{l.trip ?? '—'}</td>
                                        <td className="px-4 py-3 text-admin-muted text-xs font-mono">{l.created_at}</td>
                                    </tr>
                                );
                            })}
                        </tbody>
