...
export default function PoliceIndex({ stats, recentChecks, watchlist, departs }: Props) {
    ...
    return (
        <div className="w-full max-w-7xl space-y-6">
            <div>
                <h1 className="text-xl font-bold text-white">Interface Police & Sécurité</h1>
                <p className="text-admin-muted text-sm mt-0.5">Vérification silencieuse des passagers & listes de surveillance</p>
            </div>
            ...
            <motion.div className="grid grid-cols-4 gap-4" variants={stagger} initial="initial" animate="animate">
                {[
                    ...
                    <motion.div key={s.label} variants={fadeUp} className="bg-admin-card rounded-xl border border-white/5 p-4 flex items-center gap-3">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-admin-card rounded-xl border border-white/5 p-5">
                    <h2 className="text-sm font-semibold text-white mb-4 flex items-center gap-2"><EyeOff size={14} className="text-admin-muted" /> Vérification Silencieuse</h2>
                    ...
                    {verifyResult && (
                        <div className={`mt-4 p-3 rounded-lg ${verifyResult.match ? 'bg-status-red-bg/30 border border-status-red-border' : 'bg-status-green-bg/30 border border-status-green-border'}`}>
                            ...
                            {verifyResult.details && (
                                <p className="text-xs text-admin-muted mt-2">{verifyResult.details.reason}</p>
                            )}
                        </div>
                    )}
                </div>
                ...
                <div className="bg-admin-card rounded-xl border border-white/5 p-5">
                    <h2 className="text-sm font-semibold text-white mb-4 flex items-center gap-2"><Shield size={14} className="text-admin-muted" /> Dernières Vérifications</h2>
                        ...
                        {recentChecks.map(c => (
                            <div key={c.id} className="flex items-center justify-between text-sm py-1.5 border-b border-white/5 last:border-0">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-admin-muted text-xs">{c.phone}</span>
                                </div>
                                <span className="text-admin-muted text-xs font-mono">{c.created_at}</span>
                            </div>
                        ))}
                        {recentChecks.length === 0 && <p className="text-admin-muted text-sm">Aucune vérification.</p>}
                    </div>
                </div>

                {/* Départs du jour */}
                <div className="bg-admin-card rounded-xl border border-white/5 p-5">
                    <h2 className="text-sm font-semibold text-white mb-4">Départs à Venir</h2>
                    <div className="space-y-2 max-h-[300px] overflow-y-auto">
                        {departs.map(d => (
                            <div key={d.id} className="flex items-center justify-between text-sm py-1.5 border-b border-white/5 last:border-0">
                                <div>
                                    <p className="text-white">{d.route}</p>
                                    <p className="text-admin-muted text-xs">{d.date} {d.heure} — {d.vehicle}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-admin-muted text-xs">{d.driver}</p>
                ...
                {/* Liste surveillance */}
                <div className="bg-admin-card rounded-xl border border-white/5 p-5">
                    <h2 className="text-sm font-semibold text-white mb-4 flex items-center gap-2"><Eye size={14} className="text-admin-muted" /> Liste de Surveillance</h2>
                    <div className="space-y-2 max-h-[300px] overflow-y-auto">
                        {watchlist.map(w => (
                            <div key={w.id} className="flex items-center justify-between text-sm py-1.5 border-b border-white/5 last:border-0">
                                <div>
                                    <p className="text-white font-semibold">{w.full_name}</p>
                                    <p className="text-admin-muted text-xs">{w.phone ?? w.id_card_number ?? '—'}</p>
                                    <p className="text-status-red-text text-xs">{w.reason}</p>
                                </div>
                                <button onClick={() => router.post(route('admin.police.watchlist.clear', w.id))}
                                    className="text-admin-muted hover:text-status-green-text text-xs"
                                ><CheckCircle size={12} /></button>
                            </div>
                        ))}
                        {watchlist.length === 0 && <p className="text-admin-muted text-sm">Liste vide.</p>}
                    </div>
                </div>
            </div>
        </div>
