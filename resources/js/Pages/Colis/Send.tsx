...
export default function Send({ trips }: Props) {
    const form = useForm({
        expediteur_name: '',
        expediteur_phone: '',
        destinataire_name: '',
    ...
    const { data, setData, post, processing, errors, recentlySuccessful, transform } = form;

    transform((formData) => ({
        ...formData,
        _redirect: route('colis.send'),
    }));

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('colis.send.store'));
    };
...
Send.layout = (page: React.ReactNode) => (
    <GuestLayout title="Envoyer un Colis" activeNav="Colis">{page}</GuestLayout>
);