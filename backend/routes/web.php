...
Route::prefix('admin')->name('admin.')->middleware(['auth', 'role:directeur_general,responsable_flotte,comptable,chef_garde,guichetiere,agent_police,bagagiste'])->group(function () {
    ...
    Route::middleware('role:directeur_general')->group(function () {
        ...
        // Villes (CRUD - inspiré Yael : table normalisée)
        Route::get('/villes', [CityController::class, 'index'])->name('villes');
        Route::post('/villes', [CityController::class, 'store'])->name('villes.stocker');
        Route::put('/villes/{city}', [CityController::class, 'update'])->name('villes.mettre-a-jour');
        Route::delete('/villes/{city}', [CityController::class, 'destroy'])->name('villes.supprimer');
