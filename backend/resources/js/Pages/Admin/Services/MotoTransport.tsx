...
export default function MotoTransport({ transports, stats }: Props) {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('admin.services.moto.store'), {
            onSuccess: () => { setShowForm(false); reset(); },
        });
    };

    const ST = [
        { label: 'En attente', val: stats.en_attente, icon: Package, color: 'text-status-yellow-text', bg: 'bg-orange-900/30' },
        { label: 'En cours', val: stats.en_cours, icon: Truck, color: 'text-status-blue-text', bg: 'bg-status-blue-bg/30' },
        { label: 'Livrées', val: stats.livre, icon: CheckCheck, color: 'text-status-green-text', bg: 'bg-status-green-bg/30' },
...
MotoTransport.layout = (page: React.ReactNode) => (
    <BackOfficeLayout title="Transport motos" breadcrumbs={[{ label: 'Services' }, { label: 'Transport motos' }]}>
        {page}
    </BackOfficeLayout>
);
