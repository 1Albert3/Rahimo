...
import { storeMaintenance, completeMaintenance } from '@/api/admin';
...
export default function Maintenance() {
    const { data, loading } = useApi<MaintenanceData>('/admin/maintenance');
    ...
    const { data: form_data, setData, post, processing, reset } = useForm({
    ...
    const submit = async (e: React.FormEvent) => {
        e.preventDefault();
        await storeMaintenance(form_data as Record<string, unknown>);
        setShowForm(false);
        reset();
    };
    ...
    const ST = [
        { label: 'Planifiés', val: stats?.upcoming ?? 0, icon: Calendar, color: 'text-status-blue-text', bg: 'bg-status-blue-bg/30' },
        { label: 'En retard', val: stats?.overdue ?? 0, icon: AlertTriangle, color: 'text-status-red-text', bg: 'bg-status-red-bg/30' },
        { label: 'Terminés', val: stats?.completed ?? 0, icon: CheckCircle, color: 'text-status-green-text', bg: 'bg-status-green-bg/30' },
                                ...
                                >
                                    <div className="p-5 flex items-start justify-between gap-4">
                                        ...
                                        <div className="shrink-0 ml-3 flex items-start">
                                            <button onClick={() => handleCompleteMaintenance(o)}
                    ...
                    <div className="bg-white rounded-xl border border-outline p-6 w-full max-w-lg mx-2 shadow-xl" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={submit} className="space-y-4">
                            <div>
                                ...
                                <select value={form_data.vehicle_id} onChange={e => setData('vehicle_id', Number(e.target.value))}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select value={form_data.maintenance_type} onChange={e => setData('maintenance_type', e.target.value)}
                                ...
                                <div>
                                    ...
                                    <input type="date" value={form_data.maintenance_date} onChange={e => setData('maintenance_date', e.target.value)}
                                        className="w-full bg-gris-surface border border-outline rounded-xl px-3 py-2 text-slate-dark text-sm focus:outline-none focus:border-primary" required />
                                </div>
                            </div>
                            <div>
                                <label className="text-xs text-on-surface-variant mb-1 block">Description *</label>
                                <textarea value={form_data.description} onChange={e => setData('description', e.target.value)} required rows={2}
                                    className="w-full bg-gris-surface border border-outline rounded-xl px-3 py-2 text-slate-dark text-sm focus:outline-none focus:border-primary" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-xs text-on-surface-variant mb-1 block">Coût estimé (FCFA)</label>
                                    <input type="number" value={form_data.cost} onChange={e => setData('cost', e.target.value)}
                                        className="w-full bg-gris-surface border border-outline rounded-xl px-3 py-2 text-slate-dark text-sm focus:outline-none focus:border-primary" />
                                </div>
                                <div>
                                    <label className="text-xs text-on-surface-variant mb-1 block">Km à l'entretien</label>
                                    <input type="number" value={form_data.mileage_at_maintenance} onChange={e => setData('mileage_at_maintenance', e.target.value)}
                                        className="w-full bg-gris-surface border border-outline rounded-xl px-3 py-2 text-slate-dark text-sm focus:outline-none focus:border-primary" />
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="date" value={form_data.next_maintenance_date} onChange={e => setData('next_maintenance_date', e.target.value)}
                                        className="w-full bg-gris-surface border border-outline rounded-xl px-3 py-2 text-slate-dark text-sm focus:outline-none focus:border-primary" />
                                </div>
                                <div>
                                    <label className="text-xs text-on-surface-variant mb-1 block">Effectué par</label>
                                    <input type="text" value={form_data.performed_by} onChange={e => setData('performed_by', e.target.value)}
                                        className="w-full bg-gris-surface border border-outline rounded-xl px-3 py-2 text-slate-dark text-sm focus:outline-none focus:border-primary" />
                                </div>
                            </div>
