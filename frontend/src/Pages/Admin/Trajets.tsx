...
import { storeTrip, updateTrip, deleteTrip } from '@/api/admin';
...
export default function Trajets() {
    const { data: apiData, loading, refetch } = useApi<TrajetsData>('/admin/trajets');
    ...
    const trips = apiData?.trips ?? { data: [] } as any;
    ...
    const submit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const resp = e?.response?.data?.errors;
            if (resp) setErrors(Object.fromEntries(Object.entries(resp).map(([k, v]) => [k, (v as string[])[0]])));
        } finally {
            setProcessing(false);
        }
    };

    const confirmDelete = async (t: TripItem) => {
    ...
    return (
        <div className="w-full max-w-7xl space-y-6">
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                    <h1 className="text-xl font-bold text-slate-dark">Gestion des Trajets</h1>
                    <p className="text-on-surface-variant text-sm mt-0.5">Créer, modifier et gérer les trajets de la compagnie</p>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        ...
                        <motion.tbody variants={stagger} initial="initial" animate="animate">
                            {trips.data.length === 0 ? (
                                <tr><td colSpan={11} className="text-center py-8 text-on-surface-variant text-sm">Aucun trajet.</td></tr>
                            ) : (
                                trips.data.map((t: TripItem) => {
                                    const s = t.status;
                                    const isGreen = s === 'en_cours' || s === 'in_progress' || s === 'completed' || s === 'termine';
                                    const isYellow = s === 'scheduled';
                                    ...
                                    return (
                                    <motion.tr key={t.id} variants={fadeUp}
                                        className={`hover:bg-gris-surface transition-colors border-l-4 ${borderCls}`}
                                    >
                                        <td className="px-4 py-3 font-mono font-bold text-slate-dark text-xs">{t.trip_number}</td>
                                        <td className="px-4 py-3 text-slate-dark whitespace-nowrap">
                                            {t.departure_city} <span className="text-on-surface-variant">→</span> {t.arrival_city}
                                        </td>
                                        <td className="px-4 py-3 text-on-surface-variant">{t.departure_date}</td>
                                        <td className="px-4 py-3 text-on-surface-variant font-mono text-xs">{t.departure_time}</td>
                                        <td className="px-4 py-3 text-on-surface-variant font-mono text-xs">{t.arrival_time}</td>
                                        <td className="px-4 py-3 text-on-surface-variant">
                                            {t.booked_seats}/{t.available_seats}
                                        </td>
                                        <td className="px-4 py-3 font-semibold text-slate-dark">{formatFCFA(t.price)}</td>
                                        <td className="px-4 py-3 text-on-surface-variant text-xs">{t.vehicle ?? '—'}</td>
                                        <td className="px-4 py-3 text-on-surface-variant text-xs">{t.driver ?? '—'}</td>
            ...
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4" onClick={() => setShowModal(false)}>
                    <div className="bg-white rounded-xl border border-outline p-6 w-full max-w-xl shadow-xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={submit} className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-semibold text-on-surface-variant mb-1">Ville départ *</label>
                                ...
                                <div>
                                    <label className="block text-xs font-semibold text-on-surface-variant mb-1">Ville arrivée *</label>
                            ...
                            <div>
                                <label className="block text-xs font-semibold text-on-surface-variant mb-1">Date de départ *</label>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-semibold text-on-surface-variant mb-1">Heure départ *</label>
                                ...
                                <div>
                                    <label className="block text-xs font-semibold text-on-surface-variant mb-1">Heure arrivée *</label>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-semibold text-on-surface-variant mb-1">Prix *</label>
                                ...
                                <div>
                                    <label className="block text-xs font-semibold text-on-surface-variant mb-1">Places dispo. *</label>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-semibold text-on-surface-variant mb-1">Véhicule *</label>
                                ...
                                <div>
                                    <label className="block text-xs font-semibold text-on-surface-variant mb-1">Conducteur</label>
