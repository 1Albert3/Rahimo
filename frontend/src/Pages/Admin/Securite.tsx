...
export default function Securite() {
    const { data, loading } = useApi<{ logs: any[]; stats: any }>('/admin/securite');
    if (loading) return <div className="flex justify-center py-20 text-on-surface-variant text-sm">Chargement...</div>;
    const logs = data?.logs ?? [];
    const stats = data?.stats ?? {};

    const ST = [
        { label: 'Actions aujourd\'hui', val: stats?.aujourdhui ?? 0, icon: CalendarDays, color: 'text-status-blue-text', bg: 'bg-status-blue-bg/30' },
        { label: 'Cette semaine', val: stats?.cette_semaine ?? 0, icon: Activity, color: 'text-status-yellow-text', bg: 'bg-orange-900/30' },
        { label: 'Utilisateurs actifs', val: stats?.utilisateurs_connectes ?? 0, icon: Users, color: 'text-status-green-text', bg: 'bg-status-green-bg/30' },
    ];
