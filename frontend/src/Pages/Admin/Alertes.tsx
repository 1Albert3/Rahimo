...
interface AlerteItem { id: number; type: 'danger' | 'warning' | 'info'; categorie: string; titre: string; description: string; bus?: string; lieu?: string; temps: string; traitee: boolean; }
interface AlertesData { actives: AlerteItem[]; traitees: AlerteItem[]; stats: { critiques: number; avertissements: number; infos: number; traitees: number } }
...
export default function Alertes() {
    const { data } = useApi<AlertesData>('/admin/alertes');
    const actives = data?.actives ?? [];
    const traitees = data?.traitees ?? [];
    const stats = data?.stats ?? { critiques: 0, avertissements: 0, infos: 0, traitees: 0 };
    const STATS = [
...
function AlertesLayout({ children }: { children: React.ReactNode }) {
    ...
    const homeHref = isDriver ? '/chauffeur/trajets' : '/admin/dashboard';
    return (
        <BackOfficeLayout title="Alertes" breadcrumbs={[{ label: 'Tableau de bord', href: homeHref }, { label: 'Alertes' }]}>
            {children}
        </BackOfficeLayout>
    );
}
