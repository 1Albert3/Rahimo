...
import { updateLostItem } from '@/api/admin';
...
interface LostItemsData { items: LostItem[]; }
...
export default function LostItems() {
    const { data, loading, refetch } = useApi<LostItemsData>('/admin/objets-trouves');
    ...
    const updateStatus = (id: number, status: string) => {
        ...
        updateLostItem(id, { status, admin_notes: notes ?? '' }).then(() => refetch());
    };
    ...
    const uploadPhoto = (id: number) => {
        ...
        input.onchange = () => {
            ...
            api.post(`/admin/objets-perdus/${id}/photo`, formData).then(() => refetch());
        };
        input.click();
    };

    const ST = [
        { label: 'Perdus', val: stats?.perdu ?? 0, color: 'text-status-red-text', bg: 'bg-status-red-bg/30' },
        { label: 'Retrouvés', val: stats?.retrouve ?? 0, color: 'text-status-yellow-text', bg: 'bg-orange-900/30' },
        { label: 'Rendus', val: stats?.rendu ?? 0, color: 'text-status-green-text', bg: 'bg-status-green-bg/30' },
        { label: 'Total', val: stats?.total ?? 0, color: 'text-slate-dark', bg: 'bg-gris-surface' },
    ];
