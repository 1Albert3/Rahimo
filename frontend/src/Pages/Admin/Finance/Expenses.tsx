...
import { storeDepense } from '@/api/admin';
...
export default function Expenses() {
    const { data, loading } = useApi<ExpensesData>('/admin/depenses');

    const expenses = data?.expenses ?? { data: [] } as any;
    const [showForm, setShowForm] = useState(false);
    const form = useForm({ category: 'carburant', description: '', amount: '', expense_date: new Date().toISOString().slice(0, 10), notes: '' });

    const safeExpenses = expenses && typeof expenses === 'object' && Array.isArray(expenses.data) ? expenses : { data: [], current_page: 1, last_page: 1, per_page: 15, total: 0, links: [] };

    const submit = async (e: React.FormEvent) => {
        e.preventDefault();
        await storeDepense(form.data as Record<string, unknown>);
        setShowForm(false);
        form.reset();
    };

    const valider = async (id: number, status: string) => {
        await api.patch(`/admin/depenses/${id}/valider`, { status });
    };

    const pendingCount = safeExpenses.data.filter((e: ExpenseItem) => e.status === 'pending').length;
            ...
            >
                <div className="overflow-x-auto min-w-[700px] lg:min-w-0">
                    <table className="w-full text-sm">
                        ...
                        <tbody className="">
                            {safeExpenses.data.map((d: ExpenseItem) => {
                                const s = d.status;
