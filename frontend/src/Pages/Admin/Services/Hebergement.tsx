...
import { checkinHebergement, checkoutHebergement, storeHebergement } from '@/api/admin';
...
export default function Hebergement() {
    const { data: apiData, loading, refetch } = useApi<HebergementData>('/admin/services/hebergement');
    ...
    const ST = [
        { label: 'Réservées', val: stats?.reserve ?? 0, icon: DoorOpen, color: 'text-status-blue-text', bg: 'bg-status-blue-bg/30' },
        { label: 'En cours', val: stats?.en_cours ?? 0, icon: DoorClosed, color: 'text-status-yellow-text', bg: 'bg-orange-900/30' },
                                ...
                                <tr key={a.id} className={` hover:bg-gris-surface transition-colors border-l-4 ${borderCls}`}>
                                    ...
                                    <td className="p-4 text-center">
                                        <div className="flex items-center justify-center gap-2">
                                            {a.status === 'reserve' && (
                                                <button onClick={() => checkinHebergement(a.id).then(() => refetch())}
                                            ...
                                            {a.status === 'en_cours' && (
                                                <button onClick={() => checkoutHebergement(a.id).then(() => refetch())}
