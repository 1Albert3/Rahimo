...
interface Vehicule { id: number; registration_number: string; brand: string; model: string; latitude: number; longitude: number; speed: number; status: string; last_update: string | null; }

export default function GpsMap() {
    document.title = 'Carte GPS';
        ...
        import('leaflet').then((L) => {
            ...
            const interval = setInterval(async () => {
                try {
                    ...
                    if (data.vehicules) { setVehicules(data.vehicules); updateMarkers(data.vehicules); }
                } catch { }
            }, 15_000);

            // initial load
            api.get('/admin/flotte/gps').then(({ data }) => { if (data.vehicules) { setVehicules(data.vehicules); updateMarkers(data.vehicules); } }).catch(() => {});
