...
import { storeContrat } from '@/api/admin';
...
export default function Contrats() {
    const { data, loading } = useApi<{ contrats: ContratItem[]; employes: { id: number; name: string }[] }>('/admin/rh/contrats');
    ...
    const submit = async (e: React.FormEvent) => {
        e.preventDefault();
        await storeContrat(form.data as Record<string, unknown>);
        setShowForm(false);
        form.reset();
    };
    ...
    return (
        <div className="w-full max-w-7xl space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-xl font-bold text-slate-dark">Contrats</h1>
                    <p className="text-on-surface-variant text-sm mt-0.5">{contrats.length} contrat(s)</p>
                </div>
                        ...
                        <tbody>
                            {contrats.map(c => (
                                <tr key={c.id} className="hover:bg-gris-surface transition-colors">
                                    ...
                                    <td className="px-4 py-3 text-on-surface-variant capitalize">{TYPE_LABELS[c.type] ?? c.type}</td>
                                    <td className="px-4 py-3 text-on-surface-variant font-mono">{c.start_date}</td>
                                    <td className="px-4 py-3 text-on-surface-variant font-mono">{c.end_date ?? 'Indéterminée'}</td>
                ...
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setShowForm(false)}>
                    <div className="bg-white rounded-xl border border-outline p-6 w-full max-w-lg mx-2 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-lg font-bold text-slate-dark">Nouveau contrat</h2>
                        ...
                        <form onSubmit={submit} className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-xs text-on-surface-variant mb-1 block">Date fin (optionnelle)</label>
                                    <input type="date" value={form.data.end_date} onChange={e => form.setData('end_date', e.target.value)}
                                        className="w-full bg-gris-surface border border-outline rounded-xl px-3 py-2 text-slate-dark text-sm focus:outline-none focus:border-primary transition-colors" />
                                </div>
                                <div>
                                    <label className="text-xs text-on-surface-variant mb-1 block">Salaire de base (FCFA)</label>
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                {[
                                    ...
                                    <div key={p.k}>
                                        <label className="text-xs text-on-surface-variant mb-1 block">{p.label}</label>
                            ...
                            <div>
                                <label className="text-xs text-on-surface-variant mb-1 block">Attributions</label>
