...
const TYPE_LABELS: Record<string, string> = { vip: 'VIP Climatisé', standard: 'Standard' };
const TYPE_SUBS: Record<string, string> = { vip: 'Confort optimal, WiFi, Climatisation', standard: 'Économique, confort standard' };
const SORT_OPTIONS = [
...
export default function Search() {
    ...
    const [form, setForm] = useState({
        depart:    params.get('depart')    ?? 'Ouagadougou',
        arrivee:   params.get('arrivee')   ?? 'Bobo-Dioulasso',
        date:      params.get('date')      ?? '',
        passagers: params.get('passagers') ?? '1',
    });
    const [trajets, setTrajets] = useState<Trajet[]>([]);
    const [loading, setLoading] = useState(false);
    const [filtersOpen, setFiltersOpen] = useState(false);
    const [busType, setBusType] = useState<string[]>((params.get('bus_type') ?? 'vip,standard').split(','));
    const [priceMax, setPriceMax] = useState(params.get('price_max') ?? '');
    const [sortBy, setSortBy] = useState(`${params.get('sort_by') ?? 'departure_time'}_${params.get('sort_order') ?? 'asc'}`);

    const doSearch = async (overrides?: typeof form) => {
        const f = overrides ?? form;
        const [sortField, sortDir] = sortBy.split('_');
        setLoading(true);
        try {
            const results = await searchTrips({
                departure: f.depart, arrival: f.arrivee, date: f.date,
                passagers: parseInt(f.passagers), bus_type: busType.join(','),
                price_max: priceMax ? parseFloat(priceMax) : undefined,
                sort_by: sortField, sort_order: sortDir,
            });
            setTrajets(results as unknown as Trajet[]);
        } catch { setTrajets([]); }
        setLoading(false);
    };
    ...
    const handleSearch = (e: React.FormEvent) => { e.preventDefault(); doSearch(); };
    ...
    const selectTrip = (trajet: Trajet) => {
        ...
        navigate(`/voyages/${trajet.id}/sieges?passagers=${form.passagers}`);
    };

    return (
        <div className="w-full max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
            <form onSubmit={handleSearch} className="bg-white rounded-xl p-4 shadow-xl mb-8 flex flex-wrap gap-3 items-end">
                {[{ label: 'Départ', key: 'depart' as const }, { label: 'Arrivée', key: 'arrivee' as const }].map(({ label, key }) => (
                ...
                <button type="submit" className="bg-primary text-white px-5 py-2.5 rounded-lg text-sm font-semibold hover:brightness-110 transition-all flex items-center gap-2 shadow-xl">
                    <Filter size={16} /> Rechercher
                </button>
            </form>

            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                <button onClick={() => setFiltersOpen(true)} className="md:hidden flex items-center gap-2 text-sm font-semibold text-primary bg-white rounded-xl shadow-xl px-4 py-3">
                ...
                <aside className="hidden md:block md:col-span-3">
                    <div className="bg-white rounded-xl p-5 shadow-xl sticky top-20">
                        <div className="flex items-center justify-between mb-5">
                            ...
                            <button onClick={() => { setBusType(['vip','standard']); setPriceMax(''); setSortBy('departure_time_asc'); }} className="text-xs text-primary hover:underline font-medium">Réinitialiser</button>
                        </div>
                        ...
                        {(['vip', 'standard'] as const).map((val) => (
                            <label key={val} className="flex items-start gap-3 cursor-pointer mb-3 group">
                                <input type="checkbox" checked={busType.includes(val)} onChange={() => toggleType(val)} className="mt-0.5 w-4 h-4 rounded text-primary focus:ring-primary/20" />
                                <div>
                        ...
                        <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className={inputCls + ' mb-5'}>
                            {SORT_OPTIONS.map(opt => <option key={opt.val} value={opt.val}>{opt.label}</option>)}
                        </select>
                        <h3 className="text-xs font-semibold uppercase tracking-wider text-on-surface-variant mb-3">Prix max (FCFA)</h3>
                        <input type="number" value={priceMax} onChange={(e) => setPriceMax(e.target.value)} placeholder="Ex: 10000" className={inputCls} />
                        <button onClick={() => doSearch()} className="w-full mt-5 bg-primary text-white px-4 py-2.5 rounded-lg text-sm font-semibold hover:brightness-110 transition-all shadow-xl">Appliquer</button>
                    </div>
                </aside>
                    ...
                    {filtersOpen && (
                        ...
                        >
                            ...
                            >
                                ...
                                <button onClick={() => { doSearch(); setFiltersOpen(false); }} className="w-full bg-primary text-white px-4 py-2.5 rounded-lg text-sm font-semibold hover:brightness-110 transition-all shadow-xl">Appliquer</button>
                            </motion.div>
                        </motion.aside>
                    )}
                </AnimatePresence>
                ...
                <section className="md:col-span-9 flex flex-col gap-4">
                    <div className="bg-white rounded-xl p-5 shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                        <div>
                            <h1 className="text-xl font-bold text-slate-dark flex items-center gap-2">
                                {form.depart} <ArrowRight size={18} className="text-primary" /> {form.arrivee}
                            </h1>
                            <p className="text-sm text-on-surface-variant mt-0.5">
                                {form.date || "Aujourd'hui"} · {loading ? '...' : `${trajets.length} départ${trajets.length > 1 ? 's' : ''} trouvé${trajets.length > 1 ? 's' : ''}`}
                                {parseInt(form.passagers) > 1 && ` · ${form.passagers} passagers`}
                            </p>
                        </div>
                        <div className="flex items-center gap-2 text-xs text-on-surface-variant">
                            <ArrowUpDown size={14} />
                            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-transparent font-semibold text-slate-dark outline-none">
                                {SORT_OPTIONS.map(opt => <option key={opt.val} value={opt.val}>{opt.label}</option>)}
                            </select>
                        </div>
                    </div>
                    ...
                    {loading ? (
                        <div className="bg-white rounded-xl p-12 shadow-xl text-center">
                            <p className="text-on-surface-variant text-sm">Recherche en cours...</p>
                        </div>
                    ) : trajets.length === 0 ? (
                        <div className="bg-white rounded-xl p-12 shadow-xl text-center">
                        ...
                        <motion.div className="flex flex-col gap-4" variants={stagger} initial="initial" animate="animate">
                            {trajets.map((trajet) => {
                                ...
                                const vehicleType = (trajet as { type?: string }).type ?? 'standard';
                                return (
                                    <motion.div key={trajet.id} variants={fadeUp}
                                        className={`bg-white rounded-xl shadow-xl overflow-hidden transition-all ${complet ? 'opacity-60' : 'hover:shadow-xl cursor-pointer'}`}
                                    >
                                        <div className="p-5 flex flex-col lg:flex-row items-center gap-5">
                                            <div className="flex flex-col items-center justify-center w-full lg:w-28 pb-4 lg:pb-0 lg:pr-5 shrink-0">
                                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-2 ${vehicleType === 'vip' ? 'bg-sahel-yellow/10' : 'bg-gris-surface'}`}>
                                                ...
                                                <span className="text-[11px] font-bold uppercase tracking-wider text-center">
                                                    {vehicleType === 'vip' ? <span className="text-sahel-yellow">VIP</span> : 'Standard'}
                                                </span>
                                            </div>
                                            ...
                                            <div className="flex flex-col items-end justify-center w-full lg:w-44 pt-4 lg:pt-0 lg:pl-5 shrink-0 gap-2">
                                                <div className="flex flex-wrap gap-1.5 justify-end">
                                                    {complet ? <StatusBadge status="complet" /> : trajet.available_seats <= 5 ? (
                                                        <span className="bg-status-yellow-bg text-status-yellow-text text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-sm">{trajet.available_seats} places</span>
                                                    ) : null}
                                                </div>
                                                <div className={`text-xl font-black tracking-tight ${complet ? 'text-on-surface-variant/60 line-through' : 'text-slate-dark'}`}>{formatFCFA(trajet.price)}</div>
                                                <button onClick={() => selectTrip(trajet)} disabled={complet}
                                                    className={`w-full text-xs font-bold uppercase tracking-wider px-4 py-2.5 rounded-lg flex items-center justify-center gap-1.5 transition-all ${complet ? 'bg-gris-surface text-on-surface-variant cursor-not-allowed' : 'bg-primary text-white hover:brightness-110 shadow-xl'}`}
                                                >
                                                    {complet ? 'Non disponible' : 'Choisir'}{!complet && <ArrowRight size={13} />}
                                                </button>
                                            </div>
                                        </div>
                                    </motion.div>
