...
export default function Reclamations() {
    ...
    const { data: apiData, loading } = useApi<{ myReclamations: ReclamationItem[] }>('/services/reclamations');
    const myReclamations = apiData?.myReclamations ?? [];
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post('/services/reclamations');
    };

    const searchReclamations = () => {
        if (phoneSearch) {
            navigate('/services/reclamations?phone=' + encodeURIComponent(phoneSearch));
        }
    };
    ...
    return (
        <div className="max-w-2xl mx-auto px-4 md:px-6 py-10">
            ...
            <div className="bg-white rounded-xl shadow-xl p-5 mb-6">
                ...
                {loading && <p className="text-xs text-on-surface-variant mt-3">Chargement...</p>}
                {!loading && myReclamations.length > 0 && (
                ...
                {!loading && myReclamations.length === 0 && phoneSearch && (
