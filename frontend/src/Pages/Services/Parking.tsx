    ...
    if (recentlySuccessful) {
        return (
            <div className="max-w-lg mx-auto px-4 py-16 text-center">
                ...
                <h1 className="text-2xl font-black text-slate-dark tracking-tight mb-2">Stationnement enregistré !</h1>
                <p className="text-on-surface-variant text-sm mb-6">Veuillez vous présenter à l'accueil de la gare.</p>
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                    <Link to="/services/parking" className="px-6 py-3 bg-primary text-white rounded-xl font-bold text-sm">Nouveau stationnement</Link>
    ...
    return (
        <div className="max-w-lg mx-auto px-4 md:px-6 py-10">
            ...
            <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h1 className="text-xl font-black text-slate-dark tracking-tight">Parking</h1>
                    <p className="text-xs text-on-surface-variant">Stationnez en toute sécurité</p>
                </div>
            </div>

            <motion.form onSubmit={e => { e.preventDefault(); post('/services/parking'); }}
            ...
            >
                ...
                <button type="submit" disabled={processing}
                    className="w-full py-3 bg-blue-500 text-white rounded-xl font-bold text-sm hover:opacity-90 transition-opacity disabled:opacity-50 shadow-xl"
                >{processing ? 'Enregistrement...' : 'Enregistrer mon stationnement'}</button>
            </motion.form>

            <div className="mt-4 bg-gris-surface rounded-xl p-4 text-xs text-on-surface-variant flex items-start gap-2">
